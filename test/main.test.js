...
QUnit.test('TEST  Area Of Rectangle', assert => {
  assert.equal(mul(0, 1), 0, 'Zero')
  assert.equal(mul(10, 20), 200, 'Integers')
  assert.equal(mul(300, 200), 60000, 'long Integers')
  assert.equal(mul(-1, 200), "Error", 'Negative value for length')
  assert.equal(mul(200, -1), "Error", 'Negative value for breadth')
})
